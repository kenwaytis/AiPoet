...
import LSTM_data
import practice
from practice import Word2vec_similar
...
class MODEL():
    ...
    def train(self,checkpointsPath,reload=True):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range (epochNum):
                ...
                if globalStep % learningRateDecayStep == 0:  
                ...
                for step, (x, y) in enumerate(zip(X, Y)):
                    ...
                    if globalStep % saveStep == 0:  # prevent save at the beginning
                        print ("save model")
    ...
    def Judge_Twowords_Yayun(word1, word2):
        Yayun = word1
        f = open ("corpus/Yayun_words.txt", 'r', encoding='utf-8')
        array = []
        count=0
        while True:
            line = f.readline()
    ...
    def Judge_word_in_Yayuntable(word):
        with open ("corpus/Yayun_words.txt", 'r', encoding='utf-8') as f:
            content = f.read()
        if word in content:
            return True
        else:
        ...
        while True:
            ...
            if odd ==1:
                if MODEL.Judge_word_in_Yayuntable(re_word):
                    _import.Yayun = re_word
                    _import.Yayunlist.append(re_word)
                    print('韵脚生成中...')
                    print(_import.Yayun)
                    return re_word
            elif odd != 1 and MODEL.Judge_Twowords_Yayun(_import.Yayun,re_word) and _import.Yayun !=re_word:  #  四六八 句
                _import.Yayunlist.append(re_word)
                print('重复韵脚生成中...')
                print(re_word)
                return re_word
    ...
    def testHead(self, characters,Imbalance_words,checkpointsPath,Num,JueLv,write_continue):
        """write head poem
        ...
        gtX = tf.compat.v1.placeholder (tf.int32, shape=[1, None])  # input
        # logits Logistic模型计算结果， probs 概率值，stackCell序列化层级模型, initState初始状态, finalState最终状态
            ...
            for word in characters:
                ...
                while Everywords < Everywords_num:
                    ...
                    if word in ['不']:
                        bu_flag += 1
                    if bu_flag >=2:
                        x = np.array([[self.trainData.wordToID[before_word]]])+1
                        ...
                        if word in [']', '，', '。', ' ', '？', '！']:
                            x = np.array([[self.trainData.wordToID[word]]])+2
                            probs2, state = sess.run ([probs, finalState], feed_dict={gtX: x, initState: state})
                            word = self.probsToWord(probs2, self.trainData.words,word_list)
                        print(word)
                    '''
